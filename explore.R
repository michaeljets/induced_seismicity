...
# ca_map = get_map(location = 'CA', zoom = 6, source = "google")
# ca_plot = ggmap(ca_map)
  # geom_point(aes(x=longitude, y=latitude, col=as.factor(time_dec)), data = kern_dat)
...
ggplot() + 
  ...
  geom_point(data=cal_eq_points_df, aes(x=longitude,y=latitude), color = 'red')
  # geom_point(data=ca_wells_df2, aes(x=Longitude,y=Latitude), color = 'blue')
