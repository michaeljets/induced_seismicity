...
basic <- "https://earthquake.usgs.gov/fdsnws/event/1/"            # address of Comcat
...
while(max(counts)>maxeventspersearch){
	...
	data <- scan(datarequests[1],list(time="",latitude=0,longitude=0,depth=0,mag=0,magtype="",nst=0,gap=0,dmin=0,rms=0,net="",id="",updated="",place="",type="",horizontalError=0,depthError=0,magError=0,magNst=0,status="",locationSource="",magSource=""),sep=",",quote="\"",skip=1,quiet=TRUE)

	# if there there are 2 or more windows, then get the data for later windows and append it
	if(nwindows>1)for(i in 2:nwindows){
		data2 <- scan(datarequests[i],list(time="",latitude=0,longitude=0,depth=0,mag=0,magtype="",nst=0,gap=0,dmin=0,rms=0,net="",id="",updated="",place="",type="",horizontalError=0,depthError=0,magError=0,magNst=0,status="",locationSource="",magSource=""),sep=",",quote="\"",skip=1,quiet=TRUE)
